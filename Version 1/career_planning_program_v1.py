...
#list of all the careers available in my program
careers = [
...
while True:
       ...
       for i, career in enumerate(careers, 1): #for loop iterates over all careers in careers list and prints them line by line with a number created by enumerate
              print(f"{i}. {career.name}") #prints each career on a different line with numbers

       print("Enter the number of a career to view more information, or type '0' to quit the program.") #tells the user what to enter for different options
       choice = input("Enter number: ") #asks the user to input a number corresponding to a career or '0' to quit
       if choice == "0":
              print("Thank you for using the Career Planning Program!") #exit message
              quit() #quits the program

       elif choice.isdigit() and 1 <= int(choice) <= len(careers): #checks if the input is a digit and within the range of the careers inside the careers list
              careers[int(choice) - 1].display_info() #calls function display_info() for the career using the user's input
              input("\nPress Enter to return to the main menu") #waits for user to press enter before returning to the main menu

       else:
              print("Please enter a valid number from the list or '0' to quit.") #asks the user to enter a valid input
              input("\nPress Enter to return to the main menu") #waits for user to press enter before returning to the main menu
